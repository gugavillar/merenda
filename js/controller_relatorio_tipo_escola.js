(function () {
	'use strict';
	/*global M, angular, $*/
	function RelatorioTipoEscolaCtrl(RelatoriosResource, dateFilter, $scope) {
		var vm = this;
		...
		function gerar() {
			RelatoriosResource.tipoEscola().query({ tipo_escola: vm.dados.tipo_escola, inicio_periodo: vm.dados.inicio_periodo, fim_periodo: vm.dados.fim_periodo }).$promise.then(function (data) {
	...
	RelatorioTipoEscolaCtrl.$inject = ['RelatoriosResource', 'dateFilter', '$scope'];

	angular.module('GMERENDA').controller('RelatorioTipoEscolaCtrl', RelatorioTipoEscolaCtrl);
}());
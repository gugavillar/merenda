(function () {
	'use strict';
	/*global M, angular, $*/
	function RelatorioPorEscolaCtrl(Escolas, RelatoriosResource, dateFilter, $scope) {
		var vm = this;
		vm.escolas = Escolas;
		...
		function gerar() {
			RelatoriosResource.escolas().query({ id_escola: vm.dados.id_escola, inicio_periodo: vm.dados.inicio_periodo, fim_periodo: vm.dados.fim_periodo }).$promise.then(function (data) {
				if (data.$resolved && data.length >= 1) {
					Escolas.filter(function (elem) {
						if (elem.id_escola === vm.dados.id_escola) {
							vm.escola = elem;
						}
					});
					vm.produtos = data;
	...
	RelatorioPorEscolaCtrl.$inject = ['Escolas', 'RelatoriosResource', 'dateFilter', '$scope'];

	angular.module('GMERENDA').controller('RelatorioPorEscolaCtrl', RelatorioPorEscolaCtrl);
}());